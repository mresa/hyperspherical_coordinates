/*
 *  energia_p_orbitale_deutone.cpp
 *  Tesi
 *
 *  Created by Marco Resa on 28/05/10.
					...
					
					{
						...
						myfile_1 << " Calcolo dei misurabili per l'autovettore - coulomb:\n\n";
						{
							...
							if(Corpi.M_u() && A==3)	{
								...
								myfile_misurabili << "E_ppn = " << autovalori_c(indice_stato_fondamentale_c) + (m_A + m_B)*v3 + (4./3.)*m_B*v6<< std::endl;
								myfile_misurabili << "E_nnp = " << autovalori_c(indice_stato_fondamentale_c) + (m_A - m_B)*v3 - (4./3.)*m_B*v6<< std::endl;
							}
