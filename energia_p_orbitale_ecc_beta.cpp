...
void init_main_0(const unsigned short & A,
	...
	for (double beta = 0.4; beta <= (double)beta_max + 0.2; beta+=0.2) {
		...
		myfile_tabella_1 << "beta = " << beta<< std::endl;
		myfile_livelli << "beta = " << beta<< std::endl;
		myfile_misurabili << "beta = " << beta<< std::endl;
		myfile_misurabili_1 << "beta = " << beta<< std::endl;
...
int main (int argc, char *argv[]) {
	...
	if (argc == 1){
		std::cerr << "Sintassi: A m_max K_min K_max nucleoni P_clus masse_uguali Parita_Orbitale beta_max [modo_H]" << std::endl;
		return true;
	}
	...
	init_main_0(A,m_max,K_min,K_max,Corp,P,modo_cluster,Parita_Orbitale,beta_max);
	return true;
}
