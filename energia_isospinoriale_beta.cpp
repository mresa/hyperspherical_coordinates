...
void init_main_0(const unsigned short & A,
	...
	std::string s_ext_txt = "_beta.txt";
	std::string s_ext_dat = "_beta.dat";
	...
	{
		s = "Nuova_E_T_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot) + "_T_" + scrivi(T_max) ;
		
		s_0 = "Nuova_Tabella_E_T_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot) + "_T_" + scrivi(T_max) ;
		
		s_0_p = "Nuova_Tabella_E_T_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot) + "_T_" + scrivi(T_max) ;
		
		s_0_d = "Nuova_Tabella_E_T_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot) + "_T_" + scrivi(T_max) ;
		
		s_1 = "Nuova_E_T_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot) + "_T_" + scrivi(T_max) ;
		
		s_l = "Livelli_E_T_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot) + "_T_" + scrivi(T_max) ;
		
		s_misurabili = "Misurabili_T_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot) + "_T_" + scrivi(T_max) ;
	}
	{
	...
	for (double beta = 0.4; beta <= (double)beta_max+0.2; beta+=0.2) {
		myfile << "beta = " << beta<< std::endl;
		myfile_1 << "beta = " << beta<< std::endl;
		myfile_tabella << "beta = " << beta<< std::endl;
		myfile_tabella_p << "beta = " << beta<< std::endl;
		myfile_tabella_d << "beta = " << beta<< std::endl;
		myfile_livelli << "beta = " << beta<< std::endl;
		myfile_misurabili << "beta = " << beta<< std::endl;
		
		Energia_radiale_isospin ER ( SS, TT,N, G, HI, K_min, m_max, Corpi ,clus,beta);
		...
		for (unsigned short K_max_temp = K_min; K_max_temp <= K_max; K_max_temp+=incremento) {
			...
			Accoppiamenti_Parita BASE(KK,SS,TT,ER.D());
			int max_iter = 5*BASE.D();
			
			std::cerr << "BASE.D() "<< BASE.D() << std::endl;
			
			//		if((BASE.D()/1024)*(BASE.D()/1024)*8/1024>1){
			if(BASE.D()>11500){
				//		if(K_max_temp > 17){
				...
				
				{
					...
					mkl_free_buffers();
					ietl::lanczos<Energia_isospin,Vecspace> lanczos (EN,vec);
						...
						if (info.error_info(0)==0){
							...
							myfile_1 << " Calcolo dei misurabili per l'autovettore:\n\n";
							{
								...
								if(Corpi.M_u() && A==3)	{
									...
									myfile_tabella << eigen[0] + m_A*v3 - 2*m_B*v5 << "	";
								}
							...
							if (info.error_info(0)==0){
								...
								myfile_1 << " Calcolo dei misurabili per l'autovettore:\n\n";
								{
									...
									if(Corpi.M_u() && A==3)	{
										...
										myfile_tabella_p << eigen[0] + m_A*v3 - 2*m_B*v5 << "	";
									}
							...
							if(info.error_info(0)==0){
								...
								myfile_1 << " Calcolo dei misurabili per l'autovettore:\n\n";
								{
									...
									if(Corpi.M_u() && A==3)	{
										...
										myfile_tabella_d << eigen[0] + m_A*v3 - 2*m_B*v5 << "	";
									}
				...
				unsigned short indice_stato_fondamentale_c = 0;
				{
					...
					myfile << "Autovalori  T = " << t2.elapsed() << std::endl;
					{
						...
						myfile_1 << " Calcolo dei misurabili per l'autovettore:\n\n";
						{
							...
							if(Corpi.M_u() && A==3)	{
								tipo_matrici v5 = valor_medio(EN.MATRICI_NABLA(),autostato);
								myfile_tabella << autovalori(indice_stato_fondamentale) + m_A*v3 - 2*m_B*v5 << "	";
							}
					...
					}
					{
						...
						myfile_1 << " Calcolo dei misurabili per l'autovettore - pari\n\n";
						{
							...
							if(Corpi.M_u() && A==3)	{
								...
								myfile_tabella_p << autovalori1(indice_stato_fondamentale1) + m_A*v3 - 2*m_B*v5 << "	";
							}
					...
					}
					{
						...
						myfile_1 << " Calcolo dei misurabili per l'autovettore - dispari:\n\n";
						{
							...
							if(Corpi.M_u() && A==3)	{
								...
								myfile_tabella_d << autovalori2(indice_stato_fondamentale2) + m_A*v3 - 2*m_B*v5 << "	";
							}
...
int main (int argc, char *argv[]) {
	...
	if (argc == 1){
		std::cerr << "Sintassi: A m_max K_min K_max nucleoni S_tot T_max masse_uguali beta_max [modo_H]" << std::endl;
		return true;
	}
	...
	unsigned short beta_max = ((unsigned short) std::strtoul(argv[8+A],(char **)0, 10));
	bool modo_cluster = false;
	if (A==4) {
		beta_riferimento = 2.;
		if (argc == 10+A) {
			modo_cluster = ((bool) std::strtoul(argv[9+A],(char **)0, 10)); //mettere a 1 per avere modo H sui 4 corpi
		}				
	}
	
	std::cerr << "beta_max = " << beta_max;

	Init_Corpi_LST Corp(nucleoni,controllo);
	
	init_main_0(A,m_max,K_min,K_max,S_tot,T_max,Corp,modo_cluster,beta_max);
	return true;
}
