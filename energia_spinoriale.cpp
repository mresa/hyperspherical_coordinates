...
void init_main_0(const unsigned short & A,
	...
	{
		s = "Nuova_E_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot);
		
		s_0 = "Nuova_Tabella_E_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot);
		
		s_0_p = "Nuova_Tabella_E_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot);
		
		s_0_d = "Nuova_Tabella_E_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot);
		
		s_1 = "Nuova_E_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot);

		s_l = "Livelli_E_0_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot);

		s_misurabili = "Misurabili_" + scrivi(A) + "_" + Corpi.scrivi_corpi() + "__m_max_" + scrivi(m_max) + "_K_max_[" + scrivi(K_min) + "-" + scrivi(K_max) + "]_S_" + scrivi(S_tot);
	}
	{
	...
	for (unsigned short K_max_temp = K_min; K_max_temp <= K_max; K_max_temp+=incremento) {
		...
		Accoppiamenti_Parita BASE(KK,SS,ER.D());
		...
		if(BASE.D()>11500){
			...
			
			{
				...
				mkl_free_buffers();
				ietl::lanczos<Energia_spin,Vecspace> lanczos (EN,vec);
					...
					if (info.error_info(0)==0){
						...
						myfile_1 << " Calcolo dei misurabili per l'autovettore:\n\n";
						{
							...
							if(Corpi.M_u() && A==3)	{
								...
								myfile_misurabili << "E_ppn += " <<  (m_A + m_B)*v3 + (4./3.)*m_B*v6<< std::endl;
								myfile_misurabili << "E_nnp += " <<  (m_A - m_B)*v3 - (4./3.)*m_B*v6<< std::endl;
							}
			...
			if(BASE.D_h_p() > 0){				
				{
					...
					s_vec_temp = s_vec + s_cluster + s_K_max_temp + s_ext_txt;
					myfile_vec.open(s_vec_temp.c_str(),std::ios::in);
					s_vec_temp_1 = s_vec + s_K_max_temp + s_completa + s_ext_txt;
					myfile_vec_1.open(s_vec_temp_1.c_str(),std::ios::in);
					if (myfile_vec) {
						myfile_vec >> mygen1;
						scrivi_vettore = false;
						scrivi_vettore_p = false;						
						primopasso1 = false;
						std::cerr << "letto vec" << std::endl;
					}
					else if (myfile_vec_1) {
						myfile_vec_1 >> mygen1;
						mygen1=BASE.V_sel_p(mygen1);
						scrivi_vettore_p = true;						
						scrivi_vettore = false;
						primopasso1 = false;
						std::cerr << "letto vec_t" << std::endl;
					}
					else {
						scrivi_vettore_p = true;						
						scrivi_vettore = true;
				...
				}
				{
					...
					s_vec_temp_c = s_vec + s_cluster + s_K_max_temp + s_ext_txt;
					myfile_vec.open(s_vec_temp_c.c_str(),std::ios::in);
					s_vec_temp_c_1 = s_vec + s_K_max_temp + s_completa + s_ext_txt;
					myfile_vec_1.open(s_vec_temp_c_1.c_str(),std::ios::in);
					if (myfile_vec) {
						myfile_vec >> mygen1_c;
						scrivi_vettore_c = false;
						scrivi_vettore_pc = false;						
						primopasso1_c = false;
						std::cerr << "letto vec_t" << std::endl;
					}
					else if (myfile_vec_1) {
						myfile_vec_1 >> mygen1_c;
						mygen1_c=BASE.V_sel_p(mygen1_c);
						scrivi_vettore_c = false;
						scrivi_vettore_pc = true;						
						primopasso1_c = false;
						std::cerr << "letto vec_t" << std::endl;
					}
					else {
				...
				
				{
					...
					// call of eigenvectors function follows:   
					{
						...
						if (info.error_info(0)==0){
							...
							if (scrivi_vettore_p) {
								std::cerr << "scrivo vec p" << std::endl;
								myfile_vec.open(s_vec_temp_1.c_str(),std::ios::out);
				...
				}
				{
					...
					// call of eigenvectors function follows:   
					{
						...
						if (info.error_info(0)==0){
							...
							if (scrivi_vettore_pc) {
								std::cerr << "scrivo vec p" << std::endl;
								myfile_vec.open(s_vec_temp_c_1.c_str(),std::ios::out);
								myfile_vec << autostato << std::endl;
								myfile_vec.close();															
							}						
...
/*			if(BASE.D_h_d() > 0){
				...
				}
				{
					...
					if (myfile_vec) {
						...
						myfile_vec_1 >> mygen2_c;
						mygen2_c=BASE.V_sel_d(mygen2_c);
						scrivi_vettore_c = false;
						scrivi_vettore_pc = true;						
						primopasso2_c = false;
						std::cerr << "letto vec_t" << std::endl;
...
*/			std::cerr << "K_MAX = " << K_max_temp << " T = " << t.elapsed() << std::endl;
			...
			//V_V_MATRICI_SPARSE E (EN.MATRICI_E());	
			//V_V_MATRICI_SPARSE E_C (EN.MATRICI_E_C());	
			t1.restart();
