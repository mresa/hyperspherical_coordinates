...
void init_main_0(const unsigned short & A,
	...
	Energia_radiale_spin ER ( SS, N, G, HI, K_min, m_max, Corpi.C(), Corpi.M(), Corpi.M_R(),clus,beta);
...
int main (int argc, char *argv[]) {
	if (argc == 1){
		std::cerr << "Sintassi: A m_max K_min K_max nucleoni S_tot masse_uguali beta_max [modo_H]" << std::endl;
		return true;
	}
	...
	unsigned short beta_max = ((unsigned short) std::strtoul(argv[7+A],(char **)0, 10));
	...
	init_main_0(A,m_max,K_min,K_max,S_tot,Corp,modo_cluster,beta_max);
	return true;
}
